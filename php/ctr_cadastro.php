...
require_once __DIR__ . '/classes/CadastroUsuario.php';
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(isset($_SESSION['user_id'])){
        ...
        if(isset($_POST['user_name']) && isset($_POST['user_email']) && isset($_POST['user_password']) && isset($_POST['user_type'])){
            ...
            $user_type = $cadastro->getFuncao($_POST['user_type']);
    
            $senhaHash = password_hash($password, PASSWORD_DEFAULT);
            if($senhaHash){
                if($cadastro->registarUsuario($nome, $email, $senhaHash, $user_type['id'])){
